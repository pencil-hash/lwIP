...
3. 左侧中间分配内存流程（以框D mermp_malloc为入口）
   ...
   判断框J【池子是否耗尽】：判断取出的表头指针是否等于NULL（空指针）。
   分支J --是--> 框K【统计分配失败err加加，返回NULL】：池子所有内存都被占用，记录分配失败次数，返回空指针告诉上层申请内存失败。
...
4. 左侧下方释放内存流程（以框N memp_free为入口）
   ...
   分支R --是--> 框S【头插归还空闲链表加链表环校验】：静态池将内存块放回空闲链表表头，同时校验链表是否出现环形错误。
...
```mermaid
flowchart TD
    A["memp_init_pool"] --> B["清空空闲表头tab等于NULL"]
    B --> C["获取池子内存起始地址base"]
    C --> D{"是否遍历完所有内存块"}
    D -- "否" --> E["memp指向next等于tab 头插链表"]
    E --> F["更新表头tab等于当前内存块"]
    F --> G{"开启溢出检测"}
    G -- "是" --> H["初始化校验魔数"]
    G -- "否" --> I["跳过校验初始化"]
...
3. 连线A→B：初始化第一步，清空空闲链表表头指针。
   框B【清空空闲表头tab等于NULL】：程序刚启动，链表内没有任何空闲内存块，表头赋值为空指针NULL。
...
6. 分支D -- 否（还有未处理的内存块）
   连线D→E：执行头插链表核心逻辑。
   框E【memp指向next等于tab 头插链表】：当前这块内存的链表指针next，指向原来的空闲表头。
   连线E→F：更新表头指针。
   框F【更新表头tab等于当前内存块】：把当前处理的内存块设置为新的链表表头，实现头插，插入操作只修改两个指针，速度极快。
...
flowchart TD
    ...
    B --> C["取出空闲表头memp等于tab"]
    C --> D{"memp为空 池子耗尽"}
    D -- "是" --> E["err计数加1、开中断、返回NULL"]
    D -- "否" --> F{"开启溢出检查"}
    F -- "是" --> G["校验内存是否越界"]
    F -- "否" --> H["跳过校验"]
    G --> I["更新表头tab等于memp指向next"]
    H --> I
    I --> J["清空memp指向next防止野指针"]
    J --> K["used计数加1，更新最大占用峰值"]
    K --> L["SYS_ARCH_UNPROTECT 开中断"]
    L --> M["返回memp加MEMP_SIZE用户内存指针"]
```
...
3. 连线B→C：读取当前空闲链表表头。
   框C【取出空闲表头memp等于tab】：tab保存链表第一个空闲内存块地址，赋值给临时变量memp。

4. 连线C→D：判断池子是否存在空闲内存。
   判断框D【memp为空 池子耗尽】：如果memp等于NULL空指针，代表池子所有内存都被占用。
   分支D -- 是（无空闲内存）：连线到框E。
   框E【err计数加1、开中断、返回NULL】：记录分配失败次数，打开中断恢复系统正常运行，返回空指针告知上层内存申请失败。
...
6. 框G、框H都指向框I：执行链表弹出操作。
   框I【更新表头tab等于memp指向next】：原本表头的内存块被上层拿走使用，表头需要更新为下一块空闲内存。

7. 连线I→J：清空已分配内存块的链表指针。
   框J【清空memp指向next防止野指针】：这块内存已经离开空闲链表，残留的next指针会产生野指针，清空后方便调试排查错误。

8. 连线J→K：更新内存占用统计。
   框K【used计数加1，更新最大占用峰值】：已使用内存块数量加1，同时记录程序运行以来内存最高占用值，用来排查内存泄漏问题。

9. 连线K→L：链表操作全部完成，打开中断。
   框L【SYS_ARCH_UNPROTECT 开中断】：退出临界区，允许单片机中断正常抢占CPU。

10. 连线L→M：返回可用内存地址给上层代码。
    框M【返回memp加MEMP_SIZE用户内存指针】：重点难点！每块内存最前面是struct memp管理头，占用MEMP_SIZE字节，这段区域上层不能操作。因此地址向后偏移MEMP_SIZE字节，只把业务可读可写的内存地址返回给上层。
...
flowchart TD
    ...
    subgraph 释放主流程
        ...
        E --> G["used计数减1"]
        F --> G
        G --> H["memp指向next等于tab 头插归还链表"]
        H --> I["更新表头tab等于当前memp块"]
        I --> J{"开启链表完整性校验"}
...
5. 框E、框F共同指向框G：更新内存占用统计。
   框G【used计数减1】：归还一块内存，已使用块数减1。

6. 连线G→H：头插归还链表第一步。
   框H【memp指向next等于tab 头插归还链表】：当前要释放的内存块next指针，指向现在的空闲链表表头。

7. 连线H→I：更新空闲表头。
   框I【更新表头tab等于当前memp块】：把当前释放的内存块设置为新的链表表头，完成头插回收。
...
flowchart LR
    ...
    状态1_初始化完成 -- "memp_malloc分配" --> 状态2_分配取出C
    状态2_分配取出C -- "memp_free释放" --> 状态3_释放归还C
```
...
2. 第一块子图「状态1 初始化完成」（设备上电初始化结束后的链表状态）
...
4. 第二块子图「状态2 分配取出C」（内存C被占用后的链表状态）
